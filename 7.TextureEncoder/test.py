...
parser.add_argument('--dataroot', default="data/test/", type=str, help='path to dataset')
...
parser.add_argument("--pretrained", default="model/model_epoch_4000_iter_0.pth", type=str, help="path to pretrained model (default: none)")
...
def main():
    ...
    train_set = ImageDatasetFromFile(train_list, opt.dataroot, input_height=None, crop_height=None, output_height=opt.output_height, is_mirror=False)
    train_data_loader = torch.utils.data.DataLoader(train_set, batch_size=opt.batchSize, shuffle=False, num_workers=int(opt.workers))
    ...
    def train_vae(epoch, iteration, batch, cur_iter):  
        ...
        #vutils.save_image(torch.cat([real, rec], dim=0).data.cpu(), '{}/image_{}.jpg'.format(opt.outf, iteration),
        #                  nrow=opt.nrow)
        vutils.save_image(torch.cat([real, rec], dim=0).data.cpu(), '{}/image_{}.jpg'.format(opt.outf, iteration),
                           nrow=1)

    def test(epoch, iteration, batch, cur_iter):

        real = Variable(batch).cuda()
        real_mu, real_logvar, z, rec = model(real)
        vutils.save_image(torch.cat([real], dim=0).data.cpu(),
                          '{}/test/image_{}.jpg'.format(opt.outf, iteration),
                          nrow=1)
        vec = z.cpu().detach().numpy()
        np.savetxt('results/test/vector%s.csv' % (iteration), vec, delimiter=',')
    ...
    def train(epoch, iteration, batch, cur_iter):  
        ...
        info += 'Rec: {:.4f}, '.format(loss_rec.data.item())
        info += 'Kl_E: {:.4f}, {:.4f}, {:.4f}, '.format(lossE_real_kl.data.item(),
                                                        lossE_rec_kl.data.item(), lossE_fake_kl.data.item())
        info += 'Kl_G: {:.4f}, {:.4f}, '.format(lossG_rec_kl.data.item(), lossG_fake_kl.data.item())
       
        print(info)

        #vutils.save_image(torch.cat([real, rec, rec_2], dim=0).data.cpu(), '{}/test/image_{}.jpg'.format(opt.outf, iteration),
        #                  nrow=opt.nrow)
        vutils.save_image(torch.cat([real], dim=0).data.cpu(),
                          '{}/test/image_{}.jpg'.format(opt.outf, iteration),
                          nrow=1)
    ...
    for epoch in range(opt.start_epoch, opt.nEpochs + 1):
        ...
        for iteration, batch in enumerate(train_data_loader, 0):
            ...
            test(epoch, iteration, batch, cur_iter)
