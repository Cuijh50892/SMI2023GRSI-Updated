...
namespace TriLibCore.Samples
{
    ...
    public class AssetViewer : AssetViewerBase
    {
        ...
        public Material intMat1;
        public Material intMat2;
        public Material intMat3;
        public GameObject interactingObjectRoot;
        public List<Material> ls;
        /// <summary>
        ...
        protected override void OnMaterialsLoad(AssetLoaderContext assetLoaderContext)
        {
            ...
            while (hasPart)
            {
                string part_file_name = file_name.Substring(0, file_name.Length - 5) + i.ToString() + ".obj";
                if (File.Exists(part_file_name))
                {
                    var assetLoaderOptions = AssetLoader.CreateDefaultLoaderOptions();
                    AssetLoader.LoadModelFromFile(part_file_name, OnPartLoad, OnPartMaterialLoad, null, null, null, assetLoaderOptions);
                    i++;
                }
                else
                {
            ...
            string interJsonFile = file_name.Substring(0, file_name.Length - 12) + "interactioninfo.json";

            using (StreamReader file = File.OpenText(interJsonFile))
            {
                using (JsonTextReader reader = new JsonTextReader(file))
                {
                    JObject o = (JObject)JToken.ReadFrom(reader);
                    string I1 = o["I1"].ToString();
                    string I2 = o["I2"].ToString();
                    string I3 = o["I3"].ToString();
                    string I4 = o["I4"].ToString();
                    string I5 = o["I5"].ToString();

                    int num = 0;

                    string[] sArray = I1.Split('_');
                    string I1FileName = file_name.Substring(0, file_name.Length - 26) + "2.Interactions/" + sArray[0] + "/" + sArray[1] + "_interacting_object.obj";
                    if(File.Exists(I1FileName))
                    {
                        num++;
                        var assetLoaderOptions = AssetLoader.CreateDefaultLoaderOptions();
                        AssetLoader.LoadModelFromFile(I1FileName, OnObjectLoad, OnInteractionMaterialLoad, null, null, null, assetLoaderOptions);
                    }

                    sArray = I2.Split('_');
                    string I2FileName = file_name.Substring(0, file_name.Length - 26) + "2.Interactions/" + sArray[0] + "/" + sArray[1] + "_interacting_object.obj";
                    if (File.Exists(I2FileName))
                    {
                        num++;
                        var assetLoaderOptions = AssetLoader.CreateDefaultLoaderOptions();
                        AssetLoader.LoadModelFromFile(I2FileName, OnObjectLoad, OnInteractionMaterialLoad, null, null, null, assetLoaderOptions);
                    }

                    sArray = I3.Split('_');
                    string I3FileName = file_name.Substring(0, file_name.Length - 26) + "2.Interactions/" + sArray[0] + "/" + sArray[1] + "_interacting_object.obj";
                    if (File.Exists(I3FileName)&&num<3)
                    {
                        num++;
                        var assetLoaderOptions = AssetLoader.CreateDefaultLoaderOptions();
                        AssetLoader.LoadModelFromFile(I3FileName, OnObjectLoad, OnInteractionMaterialLoad, null, null, null, assetLoaderOptions);
                    }

                    sArray = I4.Split('_');
                    string I4FileName = file_name.Substring(0, file_name.Length - 26) + "2.Interactions/" + sArray[0] + "/" + sArray[1] + "_interacting_object.obj";
                    if (File.Exists(I4FileName) && num < 3)
                    {
                        num++;
                        var assetLoaderOptions = AssetLoader.CreateDefaultLoaderOptions();
                        AssetLoader.LoadModelFromFile(I4FileName, OnObjectLoad, OnInteractionMaterialLoad, null, null, null, assetLoaderOptions);
                    }

                    sArray = I5.Split('_');
                    string I5FileName = file_name.Substring(0, file_name.Length - 26) + "2.Interactions/" + sArray[0] + "/" + sArray[1] + "_interacting_object.obj";
                    if (File.Exists(I5FileName) && num < 3)
                    {
                        num++;
                        var assetLoaderOptions = AssetLoader.CreateDefaultLoaderOptions();
                        AssetLoader.LoadModelFromFile(I5FileName, OnObjectLoad, OnInteractionMaterialLoad, null, null, null, assetLoaderOptions);
                    }
                }
            }
        }

        public void OnObjectLoad(AssetLoaderContext assetLoaderContext)
        {
            GameObject root = assetLoaderContext.RootGameObject;
            root.transform.SetParent(interactingObjectRoot.transform);
            root.SetActive(false);
        }

        public void OnPartLoad(AssetLoaderContext assetLoaderContext)
        {
        ...
        public void OnPartMaterialLoad(AssetLoaderContext assetLoaderContext)
        {
            GameObject rootNode = assetLoaderContext.RootGameObject;
        ...
        public void OnInteractionMaterialLoad(AssetLoaderContext assetLoaderContext)
        {
            GameObject rootNode = assetLoaderContext.RootGameObject;
            foreach (Transform child in rootNode.transform)
            {
                child.GetComponent<Renderer>().material = intMat1;
            }
        }
        ...
        {
            string textureFileName = file_name.Substring(0, file_name.Length - 12) + "tex.jpg";
            byte[] texByte = File.ReadAllBytes(textureFileName);
            Texture2D tex = new Texture2D(2, 4);
            tex.LoadImage(texByte);

            Material newMat = new Material(texMat);
            string jsonfile = file_name.Substring(0, file_name.Length - 12) + "texinfo.json";
            using (StreamReader file = File.OpenText(jsonfile))
            {
                using (JsonTextReader reader = new JsonTextReader(file))
                {
                    JObject o = (JObject)JToken.ReadFrom(reader);
                    float R = (o["R"] != null) ? (float.Parse(o["R"].ToString())) : 0;
                    float G = (o["G"] != null) ? (float.Parse(o["G"].ToString())) : 0;
                    float B = (o["B"] != null) ? (float.Parse(o["B"].ToString())) : 0;
                    float A = (o["A"] != null) ? (float.Parse(o["A"].ToString())) : 0;
                    float T = (o["T"] != null) ? (float.Parse(o["T"].ToString())) : 0;
                    Color color = new Color(R/255, G / 255, B / 255, A / 255);
                    newMat.SetColor("_BaseColor",color);
                    if(T>0)
                    {
                        newMat.SetTexture("_BaseMap", tex);
                        newMat.SetTextureScale("_BaseMap", new Vector2(T, T));
                    }
                }
            }
        ...
        public void ShowFunction(int idx)
        {
            foreach (Transform child in interactingObjectRoot.transform)
            {
                child.gameObject.SetActive(false);
            }

            if(idx< interactingObjectRoot.transform.childCount)
            {
                interactingObjectRoot.transform.GetChild(idx).gameObject.SetActive(true);
            }
            
        }

        public void ShowLegend(int idx)
        {
            foreach (Transform child in interactingObjectRoot.transform)
            {
                if(child.gameObject.active)
                {
                    foreach (Transform objs in child)
                    {
                        objs.GetComponent<Renderer>().material = ls[idx];
                    }
                }
